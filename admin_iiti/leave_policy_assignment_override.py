...
class LeavePolicyAssignmentoverride(Document):
	def validate(self):
	...
	def create_leave_allocation(self, leave_type, new_leaves_allocated, leave_type_details, date_of_joining):
		...
		leave_type_name = frappe.db.get_value("Leave Type",{'leave_type':leave_type},'leave_type')
		...
		passed_months = self.get_passed_months(leave_type, new_leaves_allocated, leave_type_details, date_of_joining)
	...
	def get_passed_months(self, leave_type, new_leaves_allocated, leave_type_details, date_of_joining):
		from erpnext.hr.utils import get_monthly_earned_leave
		current_date = frappe.flags.current_date or getdate()
		if current_date > getdate(self.effective_to):
			current_date = getdate(self.effective_to)
...
def get_leave_type_details():
	...
	leave_types = frappe.get_all("Leave Type",
		fields=["name", "is_lwp", "is_earned_leave", "is_compensatory", "based_on_date_of_joining",
			"is_carry_forward", "expire_carry_forwarded_leaves_after_days", "earned_leave_frequency", "rounding","leave_type"])
