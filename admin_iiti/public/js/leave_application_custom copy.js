...
frappe.ui.form.on("Leave Application", {
	...
	to_date: function (frm) {
		...
		if (frm.doc.docstatus === 0 && frm.doc.employee && frm.doc.leave_type && frm.doc.from_date && frm.doc.to_date) {
			frappe.call({
				method: "admin_iiti.overrides.get_leave_balancess",
				args: {
					employee: frm.doc.employee,
				...
				callback: function (r) {
					if (!r.exc && r.message['total_leaves']) {
						...
						var balance = r.message['total_leaves'];
					...
					if (frm.doc.leave_type_name == "Vacation Leave") {
						...
						//vacation_period_check(frm);
						frm.trigger("vacation_period_check");
					}
				}
			});
		}
	...
	leave_encashment: function (frm) {
		...
		if (frm.doc.leave_encashment) {
			...
			//check_El_Balance(frm);
		}
	},
	vacation_period_check:function (frm) {
		frappe.call({
			method: "frappe.client.get_value",
			args: {
				doctype: "Leave Type",
